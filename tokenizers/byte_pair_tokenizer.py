...
def _most_frequent_byte_pair(str_bytes: list[int]) -> tuple[int, int]:
    """
    Find the most frequent byte pair in a given byte string.

    Args:
        str_bytes: The string as a list of integer bytes.

    Returns:
        The most frequent byte pair as a tuple of two integers.
    """
    pairs = [(str_bytes[i], str_bytes[i + 1]) for i in range(len(str_bytes) - 1)]
    pair_counts = Counter(pairs)
    most_common_pair_and_count = pair_counts.most_common(1)[0]
    return most_common_pair_and_count


def _replace_byte_pair_by_token(byte_sequence: list[int], target_pair: tuple[int, int], token: int) -> list[int]:
    """
...
def byte_pair_encode(text_bytes: list[int], max_steps: int) -> tuple[list[int], dict[tuple[int, int], int]]:
    """
    Encode a list of bytes using byte pair encoding.

    Args:
        text_bytes: The list of bytes to encode.
    ...
    for _ in range(max_steps):
        most_common_pair, count = _most_frequent_byte_pair(text_bytes)

        if count == 1:
            break

        text_bytes = _replace_byte_pair_by_token(text_bytes, most_common_pair, next_token)
        encoding_map[most_common_pair] = next_token
        next_token += 1

    return text_bytes, encoding_map
...
# vocab_size = 276
# tokenizer = BytePairTokenizer(vocab_size=vocab_size)
